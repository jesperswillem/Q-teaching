...
!!-------------------------------------------------------------------------------
  ...
  character(len=5)          :: AtSer
  character(len=1)          :: AltLoc ! not used
...
!10	format(11x,1x,a4,a4,1x,i4,4x,3f8.3)
...
23 format('>>> WARNING: Multiple GAP lines at line',i5)
  ...
  do
    ...
    if(adjustl(line) == 'GAP' .or. line(1:6) == 'TER   ') then
      ...
      !write(*, line(11:6))
      !10	format(a6,i5,1x,a4,a1,a3,1x,a1,i4,a1,3x,3f8.3)
      read(line, 10, end = 100, err = 200) RecID, AtSer,atnam_tmp, AltLoc, resnam_tmp, &
  ...
  if(nres > 0) then !avoid crashing when reading empty file
    do i = 1, lib(res(nres)%irc )%nat
      if(lib(res(nres)%irc )%atnam(i)(1:1) /='H') then
        ...
        heavy(res(nres)%start - 1 + i) = .false.
        if(atom_id(i) == 0) then
          !it was not read from the file and should be generated
          makeH(res(nres)%start - 1 + i) = .true.
        end if
      end if
    end do
...
100 format('trajectory file ', a,' frame ',i6)
...
!!-------------------------------------------------------------------------------
  ...
  !check if all solvent molecules are of the same type
  ...
  !further checks for SPC water, for which special optimizations can be used
...
120 format('Minimum distance to solute heavy atoms (solvent_pack):',f6.2,' A')
  ...
  wloop:do w_mol = 1, max_waters
    ...
    do w_at = 1, lib(irc_solvent)%nat
      ...
      !***** Petra Wennerstrom added new loop checking water-water distance
      if(use_PBC) then
...
!!-------------------------------------------------------------------------------
  ...
  do i = 1, ntor_types
    ...
    else if(tor_types(i)%tack == '' .and. tor_types(i)%tacl == '') then
...
!!  Copied from md.f90, needed when using atom masks "not restrained"
!!
!!------------------------------------------------------------------------------
! *** Local variables
  integer                          :: i,ig,i3,k
